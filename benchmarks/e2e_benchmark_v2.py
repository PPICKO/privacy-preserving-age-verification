...
class E2EPipelineBenchmark:
    ...
    def get_test_image(self):
        ...
        if self.config.test_image_path:
            ...
            print("\n  Opening webcam...")
            print("  Auto-capture when DOB >= 0.80 AND date parsing succeeds")
            print("  (Retries automatically like id_detection.py)")
            ...
            captured_frame = None
            BENCHMARK_THRESHOLD = 0.80  # Threshold for auto-capture (matches id_detection.py)
            retry_count = 0
            MAX_RETRIES = 20  # More retries than id_detection since we're benchmarking
            ...
            while True:
                ...
                if dob_detected:
                    status_text = f"DOB detected ({dob_conf:.2f}) - Verifying OCR... (attempt {retry_count + 1})"
                    color = (0, 255, 0)  # Green
                else:
                    status_text = "Waiting for DOB detection (>= 0.80)..."
                ...
                if dob_detected:
                    ...
                    print(f"  [OK] DOB detected with {dob_conf:.2f} confidence - verifying OCR...")
                    
                    # Verify OCR can parse the date (like id_detection.py retry logic)
                    can_parse = False
                    for box in results[0].boxes:
                        cls_id = int(box.cls[0].item())
                        if self.detection_config.CLASS_NAMES[cls_id] == "DOB":
                            ...
                            if crop.size > 0:
                                ...
                                if texts:
                                    cleaned = DateParser.clean_dob_text(texts, self.detection_config.DOB_STOPWORDS)
                                    parsed = DateParser.parse_date(cleaned)
                                    if parsed:
                                        can_parse = True
                                        print(f"  [OK] OCR verified: '{cleaned}' -> {parsed}")
                                        break
                                    else:
                                        print(f"  [WARN] OCR read '{cleaned}' but couldn't parse date - retrying...")
                    ...
                    if can_parse:
                        ...
                        # Don't break - continue looking for better frame
                        retry_count += 1
                        if retry_count >= MAX_RETRIES:
                            print(f"  [FAIL] Max retries ({MAX_RETRIES}) reached. Using last frame anyway.")
                            captured_frame = test_frame
                            break
                        cv2.waitKey(100)  # Small delay before retry
    ...
    def run_single_iteration(self, frame, iteration: int) -> TimingResult:
        ...
        # Note: Using 0.80 threshold (matches id_detection.py)
        BENCHMARK_THRESHOLD = 0.80  # Matches id_detection.py
        ...
        if ocr_texts:
            ...
            if parsed_date:
                ...
                print(f"    [DEBUG] [FAIL] Could not parse date from: '{cleaned_text}'")
    ...
    def run_benchmark(self):
        ...
        for box in test_result[0].boxes:
            ...
            if label == "DOB" and conf >= 0.80:  # Matches id_detection.py threshold
                dob_found = True
        
        if detected_items:
            print(f"  [OK] Detected: {', '.join(detected_items)}")
            if dob_found:
                print(f"  [OK] DOB meets threshold (>=0.80)")
            else:
